...
global unsigned patch_version = 6;
...
// is_synthetic_event returns true if the event was generated by skhd.
internal bool is_synthetic_event(CGEventRef event)
{
    int user_data = CGEventGetIntegerValueField(event, kCGEventSourceUserData);
    return user_data == SHKD_CGEVENT_USER_DATA;
}
...
internal EVENT_TAP_CALLBACK(key_handler)
{
    ...
    case kCGEventKeyDown: {
        ...
        if (is_synthetic_event(event)) {
            debug("skhd: ignoring synthetic event\n");
            return event;
        }
